...
def make_dataset(tmpdir: Path) -> Path:
    """Cria um CSV sintético de hepatite para uso nos testes."""
    rows = [
...
class TestHepatitisPredictor(unittest.TestCase):
    ...
    def test_train_persists_model_and_returns_accuracy(self):
        """Treina o modelo e garante que salva o artefato e retorna métricas."""
    ...
    def test_predict_trains_when_model_missing(self):
        """Ao chamar predict sem modelo salvo, ele deve treinar e produzir saída."""
...
class TestPredictionRepository(unittest.TestCase):
    def test_repository_disabled_without_url(self):
        """Sem URL de banco, o repositório permanece desabilitado e não falha ao logar."""
        repo = PredictionRepository(None)
        repo.log({"Age": 30}, {"prediction": 1, "label": "Live", "confidence": 0.9})
        self.assertFalse(repo.enabled)

    @unittest.skipIf(SQLALCHEMY_AVAILABLE, "Evita tocar em DB real quando SQLAlchemy está instalado")
    ...
    @unittest.skipUnless(SQLALCHEMY_AVAILABLE, "SQLAlchemy não está instalado")
    def test_repository_init_invokes_db_setup(self):
        """Quando SQLAlchemy existe, _init_db deve ser invocado na construção."""
        called = {}
